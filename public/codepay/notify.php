...
{ //业务处理例子 返回一些字符串
    ...
    $stmt->bind_param('sddissisii', $uid, $money, $price, $type, $pay_no, $param, $pay_time, $pay_tag, $status, $creat_time); //防止SQL注入
    // var_dump($stmt);
    ...
    if ($rs && $stmt->affected_rows >= 1) { //插入成功 是首次通知 可以执行业务
        ...
        // 查询是否存在该用户，如果不存在则在数据库先创建
        $link2 = mysqli_connect('localhost' , 'root' , 'root');
        if (!$link2) {
            exit('数据库连接失败');
        }
        mysqli_set_charset($link2 , 'utf8');
        mysqli_select_db($link2 , 'bw');
        $sql = "select * from codepay_user where uid = $uid";
        $result = mysqli_query($link2 , $sql);
        if (!($result && mysqli_affected_rows($link2))) {
            $sqly = "insert into codepay_user (uid , user) values($uid , '$pay_id')";
            $resulty = mysqli_query($link2 , $sqly);
        }
...
if (!$_POST['pay_no'] || md5($sign . $codepay_key) != $_POST['sign']) { //不合法的数据
    ...
    if ($result == 'ok' || $result == 'success') { //返回的是业务处理完成
        ...
        // 这里修改VIP的表并且给用户表的isvip字段修改为会员状态

        // 查询VIP表 是否有此人
        $sql1 = "select * from bw_vip where uid = $uid";
        ...
        if ($result1 && mysqli_affected_rows($link)>0) {
            ...
            // 获取到当前vip表中的id以便更新过期时间的时候用
            $id = $array[0]['id'];
            $sj = $array[0]['gqtime'];
            // 给当前过期时间叠加充值时间
            $gqtime = $sj+3600*24*$tian;
            // 更新vip表中的过期时间
            $sql2 = "update bw_vip set gqtime = $gqtime where id = $id";
            $res2 = mysqli_query($link , $sql2);
        } else {
            // 没有值的区间 创建此人的vip
            $ctime = time();
            $gqtime = $ctime+3600*24*$tian;
            $sql3 = "insert into bw_vip (uid , uname , ctime , gqtime) values($uid , '$pay_id' , $ctime , $gqtime)";
            $res3 = mysqli_query($link , $sql3);
        }
        // 更新用户VIP权限
        $sql4 = "update bw_user set isvip = 2 where id = $uid";
